...
pub struct McpEnvValues {
    pub chapter_publishing_mcp_server_bind: String,
}

/// Pure: resolve values from an explicit lookup.
pub fn load_from(lookup: impl Fn(&str) -> Option<String>) -> McpEnvValues {
    McpEnvValues {
        chapter_publishing_mcp_server_bind: lookup("CHAPTER_PUBLISHING_MCP_SERVER_BIND").filter(|value| !value.is_empty()).unwrap_or_else(|| "127.0.0.1:9090".to_string()),
    }
}
...
pub fn load_env_map(
    ...
    let mut out = std::collections::BTreeMap::new();
    let chapter_publishing_mcp_server_bind = pick(&["CHAPTER_PUBLISHING_MCP_SERVER_BIND"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, Some("127.0.0.1:9090"));
    if let Some(value) = chapter_publishing_mcp_server_bind {
        out.insert("CHAPTER_PUBLISHING_MCP_SERVER_BIND".to_string(), value);
    }
    Ok(out)
}
