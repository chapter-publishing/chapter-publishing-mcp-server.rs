...
export function loadFrom(lookup: (key: string) => string | undefined): McpEnvValues {
  return {
    chapter_publishing_mcp_server_bind: nonEmpty(lookup("CHAPTER_PUBLISHING_MCP_SERVER_BIND")) ?? "127.0.0.1:9090",
  };
}
...
export function loadEnvMap(
  ...
  const out: Record<string, string> = {};
  const chapter_publishing_mcp_server_bind = pick(["CHAPTER_PUBLISHING_MCP_SERVER_BIND"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, "127.0.0.1:9090");
  if (chapter_publishing_mcp_server_bind !== undefined) out["CHAPTER_PUBLISHING_MCP_SERVER_BIND"] = chapter_publishing_mcp_server_bind;
  return out;
}
